...
{
    public class WallaNewsSource: BaseNewsSources, INewsSource
    {
        public WallaNewsSource(Logger log) : base(log)
        {
            Log.LogEvent("Starting to receive information from the 'Walla' news site");
        ...
        public void GetTheNewsSourceCategories()
        {
            try
            {
                ...
                {
                    Log.LogError("The List of '!וואלה' categories (name of the list: 'categoriesForSite') has returned empty");
                    return;
                }
            }
            catch (Exception ex)
                    ...
                    foreach (XmlNode node in document.SelectNodes("//item"))
                    {
                        if (counter < 10)
                        {
                            ...
                            if (!string.IsNullOrEmpty(description))
                            {
                                ...
                                image = node["enclosure"].Attributes["url"].Value;

                                // Extraction of the description
                                int startIndex = description.IndexOf("<br/>") + "<br/>".Length;
                                int endIndex = description.IndexOf("</p>", startIndex);
                                resultDescription = description.Substring(startIndex, endIndex - startIndex).Trim();
                            }
